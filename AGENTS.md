...
- User decision after optimizer discussion: **do not change AdamW code/behavior for now**. Keep the current optimizer implementation unchanged.
- Next intended LLM ablation: stay with the depth-24 encoder and current depth-24 Stage 1 final, then run Stage 2 with paper/GitHub LoRA LR `2e-4`, README-aligned q/k LoRA targets (`target_modules=[q_proj,k_proj]`), train projector with `projection_lr=2e-5`, and remove auxiliary conclusion/OPD-consistency losses. This isolates Stage 2 LR/target/loss choices while leaving AdamW behavior untouched.
...
### POM (property-object match) difficulties & ceiling
POM is the weakest/most fragile LLM task; two distinct issues, neither fixed by the label correction:
- **Format fragility:** POM's scored target is object identity in the `Conclusion:` slot. Under conclusion-only loss masking the generations stopped emitting the `Conclusion:` structure and object match collapsed to `0.0` (comparison `0.0128`, superlative `0.0`). Keep **full-answer loss** (not conclusion-only) and the dedicated **POM conclusion auxiliary loss** so the format holds.
- **The encoder is NOT the bottleneck (probed 2026-06-17).** Earlier we hypothesized an "irreducible tie": `hairbrush_handle` and `ice_block` share `(H,R,T)=(2,0,0)`, so we assumed they were inseparable and that this set a POM floor. **This is empirically false at the encoder level.** `src/probe_object_separation.py` on the depth-24 seed-0 encoder shows the `(2,0,0)` pair is **perfectly separated** (1NN leave-one-out acc = 1.00; centroid cosine 0.946 but per-object cohesion 0.977/0.988). Across all 21 test pairs, 16/21 are perfectly separated and the worst is 0.90 (a single borderline press at n≈5/object). The 1024-d `feat_mean` CLS vector — exactly what is projected into one LLM token — already carries object identity near-linearly. So objects sharing a property tuple are still distinguishable by micro-texture/shape; encoder separability does not explain the POM floor.
- **Where the POM bottleneck actually is:** downstream of the encoder — the LLM/projector learning to map tactile features → object *identity* in one step (POM is recognition, not property readout), plus the format fragility above. Not the encoder representation. Implication: a CLIP-side auxiliary object/material head (Part 1) is unlikely to lift POM, since the encoder already separates objects; the higher-leverage levers are LLM-side (token interface, training the identity mapping, format stability).
- **Label-correction effect on POM:** does **not** change POM answer keys (object identity is label-independent). It does reduce generated-set degeneracy (3 test-object collisions under the old stale labels → 1 residual `(2,0,0)` label-tie), so regenerating QA on corrected labels makes POM better-posed — but the residual label-tie is not an encoder-separability problem (see above).
- **Empirical failure mode of our live run (depth-24, 3000-step Stage 2, probed 2026-06-17):** POM accuracy `0.125` (chance `0.167`) / slot `0.286` (chance `0.333`) — **at/below chance**. Diagnosed from `test_final_partial_preds.jsonl`: 100% emit a valid `Conclusion:` and descriptions are fluent (NOT a format collapse), and name emission is balanced (NOT a name-frequency bias). The model outputs the **correct candidate set** (the 3 names are given in the prompt) but **permutes the video→object slot assignment** — a pure grounding failure (can't bind a specific tactile video to a specific identity). Note the external `opd_pc_pss_pom_7b.json` model reached ~0.615 slot identity, so POM grounding is achievable; our qkv/conclusion-loss recipe regressed it.
- **Why more LoRA steps will NOT help:** training loss saturates quickly and does not track test POM. Teacher-forced next-token loss is dominated by the easy description/format tokens; the video→identity binding is a tiny, hard fraction that doesn't get learned or generalize to held-out test objects. So the optimizer plateaus on the easy part while grounding stays at chance — extra steps just sit at the plateau.
- **Aux object-head CLIP ablation (seed0, w=0.1):** `val_mean=0.620` (no regression vs baseline `0.583`); `(2,0,0)` pair separation essentially unchanged (1NN-LOO already 1.0; centroid cosine `0.946→0.929`). Confirms a CLIP-side object/material aux head is a no-op for POM — the encoder already separates objects. Feature gated default-off; `src/probe_object_separation.py` is the diagnostic.
- **Generalizing levers for POM grounding (stay in property/relational space; do NOT use direct object-naming — test objects are held out):** make the loss depend on grounding via `pom_candidate_permutation_augmentation` (currently `0`) + slot/object-frequency balance; strengthen the per-video signal to the LLM (more tokens than the single CLS vector); comparative descriptions. Not steps, not the encoder aux head.

### Paper-Explicit Vs Engineering Choices
Treat these as paper-alignment constraints: CLIP ViT-L/14, ViFi-CLIP-style frame encoding/pooling, VPT with 8 vision prompt tokens applied to **all non-embedding ViT-L/14 layers** (see prompt-depth note below), salient-frame preprocessing, 5 sampled/evaluated frames, horizontal/vertical flip `0.5`, 30 CLIP epochs, AdamW with CLIP LR `1e-3` and no weight decay, three CE-trained property classifiers, validation combined checkpoint selection, Vicuna v1.5, tactile encoder + projector + LLM, `<tact_start>` / `<tact_end>` wrapping, Stage 1 without LoRA, Stage 2 with LoRA, Stage 1 around 3200 steps, and original/reproduction LoRA centered on rank 128.

**Prompt-depth note (corrects the earlier "12 prompt layers" claim):** The original `clear-nus/octopi` configs say `prompt_depth_vision: 12`, but the upstream `utils/promptclip.py` **ignores that value** — `PromptLearningCLIPEncoderLayer` sets `add_prompt = True` for every layer with `layer_idx != 0`, and `prompt_depth_vision` is only checked for `== 0` (VPT on/off). So the released model injects VPT prompts at **all 23 non-embedding layers** of ViT-L/14, matching the paper's "each Transformer layer." Our fork's `src/utils/promptclip.py` was modified to actually honor the depth (`inject_vision = layer_idx >= total_layers - prompt_depth_vision`), so in this repo the value is live: **`prompt_depth_vision=24` reproduces the original all-layers behavior**, while `12` (last-12-layers only) is a deviation. Use `prompt_depth_vision=24` for original-faithful runs in this repo.
...
| Removed all deprecated aux losses (conclusion-only / conclusion-weight / POM-conclusion / OPD-consistency + consistency heads + `conclusion_start`/`property_labels` batch fields + config keys; permutation augmentations kept) | **Active** — decluttered 2026-06-19; verified compiles/imports/trains | `src/train_llm.py`, `src/utils/model.py`, `src/utils/dataset.py`, `configs/train_llm_config.yaml` |
| Redundant test double-pass fixed (`else` → `elif not configs["train"]`): `train=True,val=False` runs `test_final` only (was re-running `test_best` on the same final model) | **Active** | `src/train_llm.py` |

### LLM LR scheduler: our cosine over-decayed vs the original (2026-06-19)

Our scheduler starves gradient updates relative to `/tmp/orig_train_llm.py` in two ways (both **fixed/gated**):

| Change | Status | Files |
|--------|--------|-------|
| **Cosine `T_max` bug**: ours used `max_train_steps/grad_accum` (187 steps) → full cosine decay to ~0 over the short run. Original sets `T_max=len(train_loader)/grad_accum` (full epoch, 625) but breaks early at `max_train_steps`, so it only traverses ~30% of the cosine → LR stays near peak (≈80%). **Original delivered ~1.85× our total LoRA update.** | **FIXED** — `num_training_steps=int(len(train_loader)/grad_accum)` | `src/train_llm.py` |
| **Projector LR decayed (ours) vs constant (original)**: original uses 3 separate optimizers and schedules only `optimizer_llm`; projector AdamW (2e-5) + encoder SGD are unscheduled = constant. Ours used one grouped optimizer + one scheduler → decayed projector too. | **Gated** `constant_projector_lr` (separate unscheduled projector/encoder optimizer) | `src/train_llm.py` |
| `lr_schedule: cosine\|constant` — `constant` = `get_constant_schedule_with_warmup` (no decay, all groups) | **Gated**, for Stage-1-constant design | `src/train_llm.py` |
| `distinct_delimiters` (per-slot `<tact_start_a/b/c>`) and `smart_delimiter_init` (seed delimiters from real word embeddings) | **Gated, default off** — neither recovered POM binding (~chance) | `src/train_llm.py`, `src/utils/model.py` |

**Notes:** weight_decay — original `AdamW` has no `weight_decay` arg → default 0.01, **same as ours** (we MATCH the original code; both deviate from the paper's "no weight decay"). The **paper does not specify any LR schedule** — cosine is a code-only detail, and the original's is misconfigured into an effectively near-constant LR, so a **constant LR is the honest paper-consistent description**. Recommended principled design: **Stage 1 = constant LR** (from-scratch projector+embeddings, base frozen, no moving target); **Stage 2 = slight cosine decay** (high floor, not 0) for LoRA+embeddings+projector. Faithful-schedule run: `scripts/queue_faithsched_stage12.sh` (natural mean-init, no distinct/smart-init, to isolate the schedule fix).
