...
# Mean-pool each video to 1 token (=probe feat_mean) + baseline recipe. Tests the
#   "fewer clean tokens/video" fix: pool_tactile_frames=True, frames=8 -> 1 token/video.
...
LORA_EXP_ID="fp${SEED}_d24_pool1_${STEPS}_gpu5"

echo "[stage2-pool1] start $(date) | stage1=$STAGE1_DIR cuda=$STAGE2_CUDA frames=$FRAMES steps=$STEPS"
...
cleanup() { echo "[stage2-pool1] removing private config"; rm -f "$STAGE2_CONFIG"; }
trap cleanup EXIT
...
set_cfg "$STAGE2_CONFIG" \
        ...
        --key pool_tactile_frames --value True \
...
echo "[stage2-pool1] Stage 2 output: ${LORA_DIR:-<none>}"
if [ -n "${LORA_DIR:-}" ] && [ -f "$LORA_DIR/test_final_results.txt" ]; then
  echo "================ RESULT (pool1, frames=$FRAMES) ================"
