...
LORA_EXP_ID="fp${SEED}_d24_freezeproj_${STEPS}_gpu4"

echo "[stage2-freezeproj] start $(date) | stage1=$STAGE1_DIR cuda=$STAGE2_CUDA frames=$FRAMES steps=$STEPS"
...
cleanup() { echo "[stage2-freezeproj] removing private config"; rm -f "$STAGE2_CONFIG"; }
trap cleanup EXIT
...
echo "[stage2-freezeproj] Stage 2 output: ${LORA_DIR:-<none>}"
...
echo "[stage2-freezeproj] done $(date)"
