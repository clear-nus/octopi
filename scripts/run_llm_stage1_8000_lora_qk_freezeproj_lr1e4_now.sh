#!/bin/bash
# Immediate q_proj/k_proj LoRA from the Stage 1 8000 checkpoint.
# Frozen projector, lower LoRA LR 1e-4.
...
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_qk_freezeproj_lr1e4_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0001_freezeproj"

echo "[llm-qk-freezeproj-lr1e4] start $(date)"
...
trap 'echo "[llm-qk-freezeproj-lr1e4] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-qk-freezeproj-lr1e4] Stage 2: q_proj/k_proj LoRA, frozen projector, lr=1e-4"
...
echo "[llm-qk-freezeproj-lr1e4] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk-freezeproj-lr1e4] done $(date)"
