#!/bin/bash
# Original POM format with full answer loss plus extra conclusion loss.
# Runs after the current original-POM q/k control.
...
WAIT_SESSION="octopi_llm_originalpom_qk"
SEED=0
CUDA=6
CONFIG_FILE="configs/train_llm_config.yaml"
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_qk_freezeproj_original_pom_conclw05_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_conclw0.5"

echo "[llm-qk-original-pom-conclw05] queued $(date)"
echo "[llm-qk-original-pom-conclw05] waiting for tmux session ${WAIT_SESSION} to finish"
while tmux has-session -t "=${WAIT_SESSION}" 2>/dev/null; do
  sleep 120
done

echo "[llm-qk-original-pom-conclw05] start $(date)"
...
trap 'echo "[llm-qk-original-pom-conclw05] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-qk-original-pom-conclw05] Stage 2: q/k LoRA, original POM, full loss + conclusion_loss_weight=0.5"
...
echo "[llm-qk-original-pom-conclw05] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk-original-pom-conclw05] done $(date)"
