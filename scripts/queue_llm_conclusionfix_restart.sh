#!/bin/bash
# Restart LLM Stage 1 + LoRA grid after fixing conclusion_start validation/test loss.
# Uses the existing seed-0 CLIP encoder so CLIP retraining is not repeated.
...
CONFIG_BAK="${CONFIG_FILE}.conclusionfix_bak"
DATASET_PATH="dataset"
DATA_DIR="data"
ENCODER_PATH="exps/2026_06_01_09_03_40_train_clip_clip_seed_0/encoder.pt"
STAGE1_EXP_ID="full_pipeline_${SEED}_conclusionfix"

cp "$CONFIG_FILE" "$CONFIG_BAK"
trap 'echo "[llm-conclusionfix] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-conclusionfix] start $(date)"
echo "[llm-conclusionfix] regenerating data for seed ${SEED}"
rm -rf "$DATA_DIR"
...
echo "[llm-conclusionfix] Stage 1 with patched conclusion_start validation"
set_cfg --key seed --value "$SEED" \
        --key cuda --value 6 \
        ...
        --key conclusion_only_loss --value true \
...
echo "[llm-conclusionfix] Stage 1 output: $STAGE1_DIR"

for R in "${RANKS[@]}"; do
  for LR in "${LRS[@]}"; do
    ALPHA=$R
    echo "================================================================"
    echo "[llm-conclusionfix] LoRA r=${R}, alpha=${ALPHA}, lr=${LR}"
    echo "================================================================"
    ...
    export EXP_ID="full_pipeline_${SEED}_conclusionfix_lora_r${R}_lr${LR}"
    python src/train_llm.py
    echo "[llm-conclusionfix] completed LoRA r=${R}, lr=${LR}: $(latest_exp_dir "$EXP_ID")"
  done
done

echo "[llm-conclusionfix] done $(date)"
