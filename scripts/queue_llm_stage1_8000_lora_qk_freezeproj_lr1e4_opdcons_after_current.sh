#!/bin/bash
# Queue OPD consistency-loss ablation after the current q/k LoRA run.
# Same Stage 1 8000 + 8-frame encoder setup as the lr=1e-4 frozen-projector run.
...
WAIT_SESSION="octopi_llm_qk_freezeproj_lr1e4"
SEED=0
CUDA=6
CONFIG_FILE="configs/train_llm_config.yaml"
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_qk_freezeproj_lr1e4_opdcons_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0001_freezeproj_opdcons005"

echo "[llm-qk-freezeproj-lr1e4-opdcons] queued $(date)"
echo "[llm-qk-freezeproj-lr1e4-opdcons] waiting for tmux session ${WAIT_SESSION} to finish"
while tmux has-session -t "=${WAIT_SESSION}" 2>/dev/null; do
  sleep 300
done

echo "[llm-qk-freezeproj-lr1e4-opdcons] start $(date)"
...
trap 'echo "[llm-qk-freezeproj-lr1e4-opdcons] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-qk-freezeproj-lr1e4-opdcons] Stage 2: q_proj/k_proj LoRA, frozen projector, lr=1e-4, OPD consistency weight=0.05"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key opd_consistency_loss_weight --value 0.05 \
...
echo "[llm-qk-freezeproj-lr1e4-opdcons] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk-freezeproj-lr1e4-opdcons] done $(date)"
