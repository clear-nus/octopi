#!/bin/bash
# Canonical-split CLIP seed repeats for the single-change ablation:
# max_frames=8, normal single best-val checkpoint selection (no top-k averaging).
...
CONFIG_BAK="${CONFIG}.frames8_notopk_bak"
DATASET_PATH="dataset"
DATA_ROOT="/tmp/octopi_clip_frames8_notopk"
SUMMARY="clip_seed_reps_frames8_notopk_ema098.txt"
CSV="${SUMMARY}.csv"
CUDA_ID="${CUDA_ID:-7}"
SEEDS="${SEEDS:-0 1 2 3 4}"

cp "$CONFIG" "$CONFIG_BAK"
trap 'echo "[frames8-notopk] restoring config"; cp "$CONFIG_BAK" "$CONFIG"; rm -f "$CONFIG_BAK"' EXIT
...
record_result() {
  ...
  echo "[frames8-notopk] seed=${seed} ${exp_id} :: val_mean=${v} test_mean=${tm} test_combined=${tc}" | tee -a "$SUMMARY"
  echo "${seed},${exp_id},${v},${tm},${tc}" >> "$CSV"
}

run_seed() {
  local seed="$1" exp_id="clip_seed_${seed}_repro_sorted_valmean_ema098_frames8_notopk"
  local data_dir="${DATA_ROOT}/seed_${seed}"

  echo "================================================================"
  echo "[frames8-notopk] seed=${seed} :: $(date)"
  echo "================================================================"
...
print("[frames8-notopk] aggregate:")
...
echo "[frames8-notopk] done at $(date)"
