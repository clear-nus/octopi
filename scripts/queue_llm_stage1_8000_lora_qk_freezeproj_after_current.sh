#!/bin/bash
# Queue q_proj/k_proj LoRA from the Stage 1 8000 checkpoint after the
# current train_llm.py process finishes. This variant freezes the projector.
...
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_qk_freezeproj_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj"

echo "[llm-qk-freezeproj] start $(date)"
echo "[llm-qk-freezeproj] waiting for active train_llm.py process to finish"
while pgrep -f "python.*src/train_llm.py" >/dev/null; do
  sleep 60
done

cp "$CONFIG_FILE" "$CONFIG_BAK"
trap 'echo "[llm-qk-freezeproj] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-qk-freezeproj] Stage 2: q_proj/k_proj LoRA, frozen projector"
...
echo "[llm-qk-freezeproj] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk-freezeproj] done $(date)"
