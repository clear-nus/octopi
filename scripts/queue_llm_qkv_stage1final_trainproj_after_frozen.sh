#!/bin/bash
# Wait for the frozen-projector q/k/v Stage 2 run, then rerun with
# projection training enabled. Reuses prepared QA files; does not regenerate data.
...
WAIT_SESSION="${WAIT_SESSION:-octopi_llm_qkv_stage1final_gpu5}"
SEED=0
CUDA=5
CONFIG_FILE="configs/train_llm_config.yaml"
RUN_CONFIG="/tmp/octopi_train_llm_qkv_reasonconcl_pom05_stage1final_trainproj_gpu5_config.yaml"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000final_lora_qkv_r128_a256_lr0.0001_trainproj2e-5_finalonly_originalpom_reasonconcl0.25_pom0.5_cons0.02_3000_gpu5"

echo "[llm-qkv-stage1final-trainproj-gpu5] start $(date)"

while tmux has-session -t "$WAIT_SESSION" 2>/dev/null; do
  echo "[llm-qkv-stage1final-trainproj-gpu5] waiting for tmux session $WAIT_SESSION ..."
  sleep 300
done

cp "$CONFIG_FILE" "$RUN_CONFIG"
trap 'echo "[llm-qkv-stage1final-trainproj-gpu5] removing private config"; rm -f "$RUN_CONFIG"' EXIT
...
echo "[llm-qkv-stage1final-trainproj-gpu5] Stage 2: q/k/v LoRA from Stage 1 final, train projector lr=2e-5, original POM, reasoning conclusion=0.25, POM conclusion=0.5, OPD consistency=0.02, final-only eval, steps=3000"
...
echo "[llm-qkv-stage1final-trainproj-gpu5] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qkv-stage1final-trainproj-gpu5] done $(date)"
