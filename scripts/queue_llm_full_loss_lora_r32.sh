#!/bin/bash
# Single LLM LoRA run with full answer loss, reusing the existing Stage 1 checkpoint.
# This isolates the loss-target change from the failed conclusion-only LoRA run.

set -euo pipefail

source /data/samson/miniconda3/etc/profile.d/conda.sh
conda activate octopi

export TRANSFORMERS_OFFLINE=1
export HF_HUB_OFFLINE=1
export CUBLAS_WORKSPACE_CONFIG=:4096:8
export PYTHONHASHSEED=0
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True

SEED=0
R=32
LR=0.0001
ALPHA=32

CONFIG_FILE="configs/train_llm_config.yaml"
CONFIG_BAK="${CONFIG_FILE}.full_loss_lora_r32_bak"
DATASET_PATH="dataset"
DATA_DIR="data"
ENCODER_PATH="exps/2026_06_01_09_03_40_train_clip_clip_seed_0/encoder.pt"
STAGE1_DIR="exps/2026_06_01_11_25_46_train_llm_train_val_test_vicuna-7b_3200_full_pipeline_0_conclusionfix"
EXP_ID_VALUE="full_pipeline_${SEED}_fullloss_lora_r${R}_lr${LR}"

cp "$CONFIG_FILE" "$CONFIG_BAK"
trap 'echo "[llm-fullloss-r32] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT

set_cfg() {
  python src/utils/update_config.py --config_path "$CONFIG_FILE" "$@"
}

if [ ! -f "$ENCODER_PATH" ]; then
  echo "Missing encoder checkpoint: $ENCODER_PATH"
  exit 1
fi

if [ ! -f "$STAGE1_DIR/best_project.pt" ] || [ ! -f "$STAGE1_DIR/best_llm_weights.pt" ] || [ ! -d "$STAGE1_DIR/tokenizer" ]; then
  echo "Stage 1 output missing required artifacts: $STAGE1_DIR"
  exit 1
fi

echo "[llm-fullloss-r32] start $(date)"
echo "[llm-fullloss-r32] regenerating data for seed ${SEED}"
rm -rf "$DATA_DIR"
python src/utils/process_dataset.py --dataset_path "$DATASET_PATH" --output_path "$DATA_DIR" --seed "$SEED"
python src/utils/generate_qa.py --data_path "$DATA_DIR" --seed "$SEED"

set_cfg --key seed --value "$SEED" \
        --key cuda --value 6 \
        --key encoder_path --value "$ENCODER_PATH" \
        --key max_frames --value 5 \
        --key use_lora --value True \
        --key lora_trained --value False \
        --key max_train_steps --value 3000 \
        --key projection_path --value "$STAGE1_DIR/best_project.pt" \
        --key tokenizer_path --value "$STAGE1_DIR/tokenizer" \
        --key llm_path --value "$STAGE1_DIR/best_llm_weights.pt" \
        --key exps_path --value exps \
        --key freeze_projection --value False \
        --key modules_to_save --value "[embed_tokens]" \
        --key projection_lr --value "$LR" \
        --key llm_lr --value "$LR" \
        --key warmup_steps --value 50 \
        --key val_freq --value 150 \
        --key r --value "$R" \
        --key lora_alpha --value "$ALPHA" \
        --key lora_dropout --value 0.05 \
        --key target_modules --value "[q_proj, v_proj, k_proj, o_proj, gate_proj, up_proj, down_proj]" \
        --key llm_gradient_accumulation_steps --value 16 \
        --key conclusion_only_loss --value false \
        --key train --value True \
        --key val --value True \
        --key test --value True \
        --key tta_passes --value 1

export EXP_ID="$EXP_ID_VALUE"
python src/train_llm.py

echo "[llm-fullloss-r32] completed: $(find exps -maxdepth 1 -type d -name "*_${EXP_ID_VALUE}" | sort | tail -1)"
echo "[llm-fullloss-r32] done $(date)"
