#!/bin/bash
# Faithful SCHEDULE-FIX run: Stage 1 = constant LR (projector+embeddings sustained, no decay);
# Stage 2 = T_max-fixed cosine (slight decay to ~80%% of peak for LoRA+embeddings+projector).
# Natural mean-init delimiters (NO smart-init), NO distinct delimiters. Isolates the LR-schedule fix.
...
set_cfg "$STAGE1_CFG" \
        ...
        --key lr_schedule --value constant \
