#!/bin/bash
# Queue POM option-ID output-format ablation after the current OPD-consistency
# test-only evaluation finishes.
...
WAIT_SESSION="octopi_llm_opdcons005_test"
SEED=0
CUDA=6
CONFIG_FILE="configs/train_llm_config.yaml"
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_qk_freezeproj_pom_option_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_balval_pomopt"

echo "[llm-pom-option] queued $(date)"
echo "[llm-pom-option] waiting for tmux session ${WAIT_SESSION} to finish"
while tmux has-session -t "=${WAIT_SESSION}" 2>/dev/null; do
  sleep 120
done

echo "[llm-pom-option] start $(date)"
...
trap 'echo "[llm-pom-option] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-pom-option] Stage 2: q/k LoRA, frozen projector, balanced val checkpoint, POM option-ID format"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key val_checkpoint_metric --value balanced_task_loss \
        --key r --value 128 \
...
echo "[llm-pom-option] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-pom-option] done $(date)"
