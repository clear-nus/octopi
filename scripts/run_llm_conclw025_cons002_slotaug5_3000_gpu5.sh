#!/bin/bash
# Current best LLM recipe plus all non-original slot permutations for POM/PSS.
# Does not regenerate QA files; augmentation is applied in the Dataset.
...
RUN_CONFIG="/tmp/octopi_train_llm_slotaug5_3000_gpu5_config.yaml"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_conclw0.25_cons0.02_slotaug5_3000_gpu5"

echo "[llm-conclw025-cons002-slotaug5-3000-gpu5] start $(date)"

cp "$CONFIG_FILE" "$RUN_CONFIG"
trap 'echo "[llm-conclw025-cons002-slotaug5-3000-gpu5] removing private config"; rm -f "$RUN_CONFIG"' EXIT
...
echo "[llm-conclw025-cons002-slotaug5-3000-gpu5] Stage 2: q/k LoRA, original POM, conclusion_loss_weight=0.25, OPD consistency=0.02, slot aug=5, steps=3000"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key multi_object_slot_permutation_augmentation --value 5 \
...
echo "[llm-conclw025-cons002-slotaug5-3000-gpu5] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-conclw025-cons002-slotaug5-3000-gpu5] done $(date)"
