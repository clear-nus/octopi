#!/bin/bash
# Stabilization run after the final-vs-best conclusion-weighted repeat.
# Uses milder conclusion weighting plus light OPD consistency loss.
...
CONFIG_BAK="${CONFIG_FILE}.original_pom_conclw025_cons002_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_conclw0.25_cons0.02_finaleval"

echo "[llm-qk-original-pom-conclw025-cons002] queued $(date)"
echo "[llm-qk-original-pom-conclw025-cons002] waiting for tmux session ${WAIT_SESSION} to finish"
while tmux has-session -t "=${WAIT_SESSION}" 2>/dev/null; do
  sleep 120
done

echo "[llm-qk-original-pom-conclw025-cons002] start $(date)"
...
trap 'echo "[llm-qk-original-pom-conclw025-cons002] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-qk-original-pom-conclw025-cons002] Stage 2: q/k LoRA, original POM, conclusion_loss_weight=0.25, OPD consistency=0.02"
...
echo "[llm-qk-original-pom-conclw025-cons002] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk-original-pom-conclw025-cons002] done $(date)"
