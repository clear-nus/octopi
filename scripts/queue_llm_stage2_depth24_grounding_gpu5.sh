...
# One-factor change vs the current depth-24 Stage 2 run:
#   pom_candidate_permutation_augmentation: 0 -> 5
# Tests whether forcing POM grounding (breaking the candidate-list-order shortcut)
# moves object-match off its at/below-chance floor. Everything else identical.
# Reuses prepared QA files; refuses to regenerate data.
...
STAGE2_CONFIG="/tmp/octopi_stage2_depth24_candperm5_${SEED}_${STAGE2_CUDA}.yaml"
...
# Existing depth-24 Stage 1 final (8000 steps, finalonly), reused as-is.
...
CANDPERM="${CANDPERM:-5}"
SLOTPERM="${SLOTPERM:-0}"
LABELPERM="${LABELPERM:-0}"
STEPS="${STEPS:-5000}"
# Short exp id: train_llm prepends a long auto prefix, so keep this terse (255-char path limit).
LORA_EXP_ID="fp${SEED}_d24_cp${CANDPERM}_sp${SLOTPERM}_lp${LABELPERM}_tb_${STEPS}_gpu5"

echo "[stage2-candperm] start $(date)"
echo "[stage2-candperm] stage1_dir=$STAGE1_DIR"
echo "[stage2-candperm] cuda=$STAGE2_CUDA candperm=$CANDPERM"
...
cleanup() { echo "[stage2-candperm] removing private config"; rm -f "$STAGE2_CONFIG"; }
trap cleanup EXIT
...
set_cfg "$STAGE2_CONFIG" \
        ...
        --key task_balanced_training --value True \
        ...
        --key pom_candidate_permutation_augmentation --value "$CANDPERM" \
        --key multi_object_slot_permutation_augmentation --value "$SLOTPERM" \
        --key multi_object_label_order_permutation_augmentation --value "$LABELPERM" \
...
echo "[stage2-candperm] Stage 2 output: ${LORA_DIR:-<none>}"
if [ -n "${LORA_DIR:-}" ] && [ -f "$LORA_DIR/test_final_results.txt" ]; then
  echo "================ POM RESULT (candperm=$CANDPERM) ================"
  grep -aiE "object_match|accuracy|slot" "$LORA_DIR/test_final_results.txt" | head -12
fi
echo "[stage2-candperm] done $(date)"
