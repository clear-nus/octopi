#!/bin/bash
# Paper-closer LLM Stage 1 + Stage 2 run.
# Stage 1 uses full answer loss. Stage 2 uses the original-ish LoRA setting:
# r=128, alpha=128, lr=2e-4, warmup=20.
...
CONFIG_BAK="${CONFIG_FILE}.paperclose_full_stage12_bak"
DATASET_PATH="dataset"
DATA_DIR="/tmp/octopi_llm_paperclose_full_stage12_data"
ENCODER_PATH="exps/2026_06_01_09_03_40_train_clip_clip_seed_0/encoder.pt"
STAGE1_EXP_ID="full_pipeline_${SEED}_paperclose_fullloss_stage1"
LORA_EXP_ID="full_pipeline_${SEED}_paperclose_fullloss_lora_r128_lr0.0002"

cp "$CONFIG_FILE" "$CONFIG_BAK"
trap 'echo "[llm-paperclose] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
if tmux ls 2>/dev/null | grep -q '^octopi_llm_fullloss_r32:'; then
  echo "[llm-paperclose] waiting for octopi_llm_fullloss_r32 to finish before touching config/data"
fi
while tmux ls 2>/dev/null | grep -q '^octopi_llm_fullloss_r32:'; do
  sleep 60
done

echo "[llm-paperclose] start $(date)"
echo "[llm-paperclose] regenerating data for seed ${SEED} in ${DATA_DIR}"
rm -rf "$DATA_DIR"
...
echo "[llm-paperclose] Stage 1: full answer loss, no LoRA"
...
echo "[llm-paperclose] Stage 1 output: $STAGE1_DIR"

echo "[llm-paperclose] Stage 2: paper-ish LoRA r=128 lr=2e-4 warmup=20"
set_cfg --key data_dir --value "$DATA_DIR" \
        --key seed --value "$SEED" \
        --key cuda --value 6 \
        --key encoder_path --value "$ENCODER_PATH" \
...
echo "[llm-paperclose] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-paperclose] done $(date)"
