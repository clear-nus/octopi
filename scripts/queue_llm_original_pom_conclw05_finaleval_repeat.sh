#!/bin/bash
# Repeat the original-POM conclusion-weighted run after the current one, using
# the updated train_llm.py that saves/evaluates both final and best-val checkpoints.
...
WAIT_SESSION="octopi_llm_originalpom_conclw05"
SEED=0
CUDA=6
CONFIG_FILE="configs/train_llm_config.yaml"
CONFIG_BAK="${CONFIG_FILE}.original_pom_conclw05_finaleval_repeat_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_conclw0.5_finaleval"

echo "[llm-qk-original-pom-conclw05-finaleval] queued $(date)"
echo "[llm-qk-original-pom-conclw05-finaleval] waiting for tmux session ${WAIT_SESSION} to finish"
while tmux has-session -t "=${WAIT_SESSION}" 2>/dev/null; do
  sleep 120
done

echo "[llm-qk-original-pom-conclw05-finaleval] start $(date)"
...
trap 'echo "[llm-qk-original-pom-conclw05-finaleval] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-qk-original-pom-conclw05-finaleval] Stage 2: q/k LoRA, original POM, full loss + conclusion_loss_weight=0.5, final+best eval"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key conclusion_loss_weight --value 0.5 \
...
echo "[llm-qk-original-pom-conclw05-finaleval] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk-original-pom-conclw05-finaleval] done $(date)"
