#!/bin/bash
# Queue q/k/v LoRA with reasoning-only conclusion loss after the current label-order run.
...
WAIT_SESSION="octopi_llm_qk_reasonconcl_pom05_gpu5"
while tmux has-session -t "$WAIT_SESSION" 2>/dev/null; do
  echo "[llm-qkv-reasonconcl-pom05-gpu5] waiting for tmux session $WAIT_SESSION at $(date)"
  sleep 300
done
...
RUN_CONFIG="/tmp/octopi_train_llm_qkv_reasonconcl_pom05_gpu5_config.yaml"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qkv_r128_a256_lr0.0001_freezeproj_finalonly_originalpom_reasonconcl0.25_pom0.5_cons0.02_3000_gpu5"

echo "[llm-qkv-reasonconcl-pom05-gpu5] start $(date)"

cp "$CONFIG_FILE" "$RUN_CONFIG"
trap 'echo "[llm-qkv-reasonconcl-pom05-gpu5] removing private config"; rm -f "$RUN_CONFIG"' EXIT
...
echo "[llm-qkv-reasonconcl-pom05-gpu5] Stage 2: q/k/v LoRA, original POM, reasoning conclusion=0.25, POM conclusion=0.5, OPD consistency=0.02, final-only eval, steps=3000"
...
echo "[llm-qkv-reasonconcl-pom05-gpu5] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qkv-reasonconcl-pom05-gpu5] done $(date)"
