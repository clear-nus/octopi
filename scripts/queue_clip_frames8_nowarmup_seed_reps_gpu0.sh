#!/bin/bash
# Canonical-split CLIP seed repeats for depth-24 + no-warmup + val-combined
# checkpoint selection.
# Matches the depth-24 8-frame encoder config except:
#   warmup_ratio=0.0 and val_checkpoint_metric=val_combined
# Waits for the current depth-24 GPU-0 sweep before mutating train_clip_config.yaml.
...
WAIT_SESSION="${WAIT_SESSION:-octopi_clip_frames8_depth24_gpu0}"
CONFIG="configs/train_clip_config.yaml"
CONFIG_BAK="${CONFIG}.frames8_depth24_nowarmup_valcombined_gpu0_bak"
DATASET_PATH="dataset"
DATA_ROOT="/tmp/octopi_clip_frames8_depth24_nowarmup_valcombined_gpu0"
SUMMARY="clip_seed_reps_frames8_depth24_nowarmup_valcombined_ema098_gpu0.txt"
CSV="${SUMMARY}.csv"
CUDA_ID="${CUDA_ID:-0}"
SEEDS="${SEEDS:-0 1 2 3 4}"

echo "[frames8-depth24-nowarmup-valcombined-gpu0] queued $(date)"
while tmux has-session -t "$WAIT_SESSION" 2>/dev/null; do
  echo "[frames8-depth24-nowarmup-valcombined-gpu0] waiting for tmux session $WAIT_SESSION ..."
  sleep 300
done

cp "$CONFIG" "$CONFIG_BAK"
trap 'echo "[frames8-depth24-nowarmup-valcombined-gpu0] restoring config"; cp "$CONFIG_BAK" "$CONFIG"; rm -f "$CONFIG_BAK"' EXIT
...
latest_log_field() {
  ...
  python src/utils/parse_clip_log.py "$dir/log.txt" --selector val_combined --field "$field"
}
...
record_result() {
  ...
  echo "[frames8-depth24-nowarmup-valcombined-gpu0] seed=${seed} ${exp_id} :: val_mean=${v} test_mean=${tm} test_combined=${tc}" | tee -a "$SUMMARY"
  echo "${seed},${exp_id},${v},${tm},${tc}" >> "$CSV"
}

run_seed() {
  local seed="$1" exp_id="clip_seed_${seed}_repro_sorted_valcombined_ema098_frames8_depth24_nowarmup_gpu0"
  local data_dir="${DATA_ROOT}/seed_${seed}"

  echo "================================================================"
  echo "[frames8-depth24-nowarmup-valcombined-gpu0] seed=${seed} :: $(date)"
  echo "================================================================"
...
print("[frames8-depth24-nowarmup-valcombined-gpu0] aggregate:")
...
echo "[frames8-depth24-nowarmup-valcombined-gpu0] done at $(date)"
