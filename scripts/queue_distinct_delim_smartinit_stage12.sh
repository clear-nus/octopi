#!/bin/bash
# Stage 1 -> Stage 2 with DISTINCT per-slot delimiters + SMART INIT (seed <tact_start_a>
# from mean of "start"+"a" word embeddings, etc.) so slots START separated/meaningful
# instead of collapsing to one identical mean vector. Probe showed mean-init slots only
# reach cos~0.88; this aims to fix that. Reuses the chosen f5 sinusoid encoder.
# Tests whether unique, learnable per-block addresses fix POM video->object binding.
# Reuses the chosen sinusoid f5 encoder (binding is LLM-side; encoder unchanged).
...
TAG="${TAG:-distinctdelim_smartinit_f${FRAMES}}"
...
set_cfg "$STAGE1_CFG" \
        ...
        --key distinct_delimiters --value True \
