#!/bin/bash
# LoRA target-module ablation after the current POM option/loss run.
# Expands q/k LoRA to attention-only q/k/v/o while keeping the rest fixed.
...
WAIT_SESSION="octopi_llm_pomopt_loss"
SEED=0
CUDA=6
CONFIG_FILE="configs/train_llm_config.yaml"
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_attnall_freezeproj_pom_option_loss_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qkvo_r128_a256_lr0.0002_freezeproj_lossval_pomopt"

echo "[llm-pom-option-qkvo] queued $(date)"
echo "[llm-pom-option-qkvo] waiting for tmux session ${WAIT_SESSION} to finish"
while tmux has-session -t "=${WAIT_SESSION}" 2>/dev/null; do
  sleep 120
done

echo "[llm-pom-option-qkvo] start $(date)"
...
trap 'echo "[llm-pom-option-qkvo] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-pom-option-qkvo] Stage 2: q/k/v/o LoRA, frozen projector, loss checkpoint, POM option-ID format"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key target_modules --value "[q_proj, k_proj, v_proj, o_proj]" \
        --key llm_gradient_accumulation_steps --value 16 \
        --key conclusion_only_loss --value false \
        --key pom_option_id_format --value true \
...
echo "[llm-pom-option-qkvo] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-pom-option-qkvo] done $(date)"
