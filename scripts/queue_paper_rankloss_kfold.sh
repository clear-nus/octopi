#!/bin/bash
# Paper-defaults + scaled weighted CE + decoupled heads, with ranking loss restored.
# Runs a full 5-fold k-fold and deletes each fold's vificlip.pt after metrics parse.
...
echo "================= paper defaults + scaled + decoupled + ranking ================="
...
EXP_TAG=paper_cbs_decoupled_rank05 DELETE_VIFICLIP=1 bash scripts/run_clip_kfold.sh
cp clip_kfold_summary.txt clip_kfold_summary_paper_cbs_decoupled_rank05.txt
cp clip_kfold_summary.txt.csv clip_kfold_summary_paper_cbs_decoupled_rank05.txt.csv
