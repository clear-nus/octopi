#!/bin/bash
# Isolated task-balanced Stage 2 run.
# Same full-answer CE objective and original POM wording, but samples each
# gradient-accumulation window as 4 OPD / 4 POM / 4 PC / 4 PSS.
...
CONFIG_BAK="${CONFIG_FILE}.task_balanced_full_loss_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_taskbalanced"

echo "[llm-task-balanced-full-loss] start $(date)"
...
trap 'echo "[llm-task-balanced-full-loss] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-task-balanced-full-loss] Stage 2: q/k LoRA, original POM, full loss, task-balanced training"
...
echo "[llm-task-balanced-full-loss] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-task-balanced-full-loss] done $(date)"
