#!/bin/bash
# Queue POM-only conclusion-loss run after the current label-order run.
# This does not regenerate QA files; it reuses the existing prepared QA files.
...
while tmux has-session -t "$WAIT_SESSION" 2>/dev/null; do
  echo "[llm-pom-conclw025-3000-gpu5] waiting for tmux session $WAIT_SESSION at $(date)"
  sleep 300
done
...
RUN_CONFIG="/tmp/octopi_train_llm_pom_conclw025_3000_gpu5_config.yaml"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_pomconclw0.25_cons0.02_3000_gpu5"

echo "[llm-pom-conclw025-3000-gpu5] start $(date)"

cp "$CONFIG_FILE" "$RUN_CONFIG"
trap 'echo "[llm-pom-conclw025-3000-gpu5] removing private config"; rm -f "$RUN_CONFIG"' EXIT
...
echo "[llm-pom-conclw025-3000-gpu5] Stage 2: q/k LoRA, original POM, POM-only conclusion_loss_weight=0.25, OPD consistency=0.02, no slot/label augmentation, steps=3000"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key pom_conclusion_loss_weight --value 0.25 \
...
echo "[llm-pom-conclw025-3000-gpu5] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-pom-conclw025-3000-gpu5] done $(date)"
