#!/bin/bash
# Current best LLM recipe plus train-time slot permutation augmentation for
# POM/PSS. Does not regenerate QA files; augmentation is applied in the Dataset.
...
CONFIG_BAK="${CONFIG_FILE}.conclw025_cons002_slotaug2_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_conclw0.25_cons0.02_slotaug2"

echo "[llm-conclw025-cons002-slotaug2] start $(date)"
...
trap 'echo "[llm-conclw025-cons002-slotaug2] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-conclw025-cons002-slotaug2] Stage 2: q/k LoRA, original POM, conclusion_loss_weight=0.25, OPD consistency=0.02, slot aug=2"
...
echo "[llm-conclw025-cons002-slotaug2] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-conclw025-cons002-slotaug2] done $(date)"
