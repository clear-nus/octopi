#!/bin/bash
# Canonical-split CLIP seed repeats for the depth-24 VPT encoder, but using the
# OLD (pre-correction) commented property labels:
#   microfiber_cloth hardness 1->0 (soft), bath_towel hardness 1->0 (soft),
#   clothes_peg roughness 1->0 (smooth).
# Everything else matches scripts/queue_clip_frames8_depth24_seed_reps_gpu0.sh.
# Backs up + patches constants.py and the config, restores both on exit.
...
CONFIG_BAK="${CONFIG}.frames8_depth24_oldlabels_gpu0_bak"
CONSTANTS="src/utils/constants.py"
CONSTANTS_BAK="${CONSTANTS}.frames8_depth24_oldlabels_gpu0_bak"
DATASET_PATH="dataset"
DATA_ROOT="/tmp/octopi_clip_frames8_depth24_oldlabels_gpu0"
SUMMARY="clip_seed_reps_frames8_depth24_oldlabels_ema098_gpu0.txt"
CSV="${SUMMARY}.csv"
CUDA_ID="${CUDA_ID:-0}"
...
trap 'echo "[frames8-depth24-oldlabels-gpu0] restoring config + constants"; cp "$CONFIG_BAK" "$CONFIG"; rm -f "$CONFIG_BAK"; cp "$CONSTANTS_BAK" "$CONSTANTS"; rm -f "$CONSTANTS_BAK"' EXIT

# --- Revert the three corrected labels back to the old commented values. ---
python - "$CONSTANTS" <<'PY'
import sys
path = sys.argv[1]
src = open(path).read()
...
patches = [
    # HARDNESS_RANK: microfiber_cloth + bath_towel 1 -> 0 (drop the # 0 comments)
    ("    # 'physiclear_microfiber_cloth': 0,\n"
     ...
     "    'physiclear_microfiber_cloth': 0,\n"
     "    'physiclear_bath_towel': 0,\n"),
    # ROUGHNESS_RANK: clothes_peg 1 -> 0
...
for old, new in patches:
    if old not in src:
        sys.exit(f"PATCH FAILED: expected block not found:\n{old}")
    src = src.replace(old, new, 1)

open(path, "w").write(src)
print("[oldlabels] constants.py patched to old labels:")
print("  HARDNESS_RANK microfiber_cloth=0, bath_towel=0")
print("  ROUGHNESS_RANK clothes_peg=0")
PY
...
record_result() {
  ...
  echo "[frames8-depth24-oldlabels-gpu0] seed=${seed} ${exp_id} :: val_mean=${v} test_mean=${tm} test_combined=${tc}" | tee -a "$SUMMARY"
  echo "${seed},${exp_id},${v},${tm},${tc}" >> "$CSV"
}

run_seed() {
  local seed="$1" exp_id="clip_seed_${seed}_repro_sorted_valmean_ema098_frames8_depth24_oldlabels_gpu0"
  local data_dir="${DATA_ROOT}/seed_${seed}"

  echo "================================================================"
  echo "[frames8-depth24-oldlabels-gpu0] seed=${seed} :: $(date)"
  echo "================================================================"
...
print("[frames8-depth24-oldlabels-gpu0] aggregate:")
...
echo "[frames8-depth24-oldlabels-gpu0] done at $(date)"
