#!/bin/bash
# CLIP depth-24 encoder + auxiliary OBJECT-instance representation-shaping head.
# Identical to scripts/queue_clip_frames8_depth24_seed_reps_gpu0.sh except it adds:
#   aux_classifier_target=object, aux_loss_weight=${AUX_WEIGHT:-0.1}
# The aux head is CLIP-training only (discarded for the LLM); it pushes the encoder
# to separate objects that share the same (hardness,roughness,texture) tuple.
# Default: single seed 0 (fast ablation). Override with SEEDS="0 1 2 3 4".
# Uses corrected labels (current constants.py); does NOT touch constants.py.
...
CONFIG_BAK="${CONFIG}.frames8_depth24_auxobject_gpu0_bak"
DATASET_PATH="dataset"
DATA_ROOT="/tmp/octopi_clip_frames8_depth24_auxobject_gpu0"
SUMMARY="clip_seed_reps_frames8_depth24_auxobject_ema098_gpu0.txt"
CSV="${SUMMARY}.csv"
CUDA_ID="${CUDA_ID:-0}"
SEEDS="${SEEDS:-0}"
...
trap 'echo "[auxobject-gpu0] restoring config"; cp "$CONFIG_BAK" "$CONFIG"; rm -f "$CONFIG_BAK"' EXIT
...
configure_variant() {
  ...
  set_cfg --key data_dir --value "$data_dir" \
          ...
          --key aux_classifier_target --value object \
          --key aux_loss_weight --value "$AUX_WEIGHT" \
...
record_result() {
  ...
  echo "[auxobject-gpu0] seed=${seed} ${exp_id} :: val_mean=${v} test_mean=${tm} test_combined=${tc}" | tee -a "$SUMMARY"
  echo "${seed},${exp_id},${v},${tm},${tc}" >> "$CSV"
}

run_seed() {
  local seed="$1" exp_id="clip_seed_${seed}_repro_sorted_valmean_ema098_frames8_depth24_auxobject_w${AUX_WEIGHT}_gpu0"
  local data_dir="${DATA_ROOT}/seed_${seed}"

  echo "================================================================"
  echo "[auxobject-gpu0] seed=${seed} aux_weight=${AUX_WEIGHT} :: $(date)"
  echo "================================================================"
...
print("[auxobject-gpu0] aggregate:")
...
echo "[auxobject-gpu0] done at $(date)"
