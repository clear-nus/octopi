...
FRAMES="${FRAMES:-8}"
STEPS="${STEPS:-3000}"
STAGE2_CUDA="${STAGE2_CUDA:-6}"
STAGE2_GPU_CONFIG="${STAGE2_GPU_CONFIG:-configs/gpu_config_7b_gpu6.json}"
...
LORA_EXP_ID="fp${SEED}_d24_trainallemb_${STEPS}_gpu6"

echo "[stage2-trainallemb] start $(date) | stage1=$STAGE1_DIR cuda=$STAGE2_CUDA frames=$FRAMES steps=$STEPS"
...
cleanup() { echo "[stage2-trainallemb] removing private config"; rm -f "$STAGE2_CONFIG"; }
trap cleanup EXIT
...
echo "[stage2-trainallemb] Stage 2 output: ${LORA_DIR:-<none>}"
...
echo "[stage2-trainallemb] done $(date)"
