#!/bin/bash
# q/k/v/o LoRA target ablation with lower LR.
# Same as the q/k/v/o POM-option loss-val run, except llm_lr=1e-4.
...
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_attnall_freezeproj_pom_option_loss_lr1e4_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qkvo_r128_a256_lr0.0001_freezeproj_lossval_pomopt"

echo "[llm-pom-option-qkvo-lr1e4] start $(date)"
...
trap 'echo "[llm-pom-option-qkvo-lr1e4] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-pom-option-qkvo-lr1e4] Stage 2: q/k/v/o LoRA, frozen projector, loss checkpoint, POM option-ID format, lr=1e-4"
...
echo "[llm-pom-option-qkvo-lr1e4] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-pom-option-qkvo-lr1e4] done $(date)"
