#!/bin/bash
# Format-only POM ablation: keep option-ID POM answers, but select checkpoints
# by ordinary validation loss instead of task-balanced validation loss.
...
CONFIG_BAK="${CONFIG_FILE}.stage1_8000_qk_freezeproj_pom_option_loss_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_pomopt"

echo "[llm-pom-option-loss] start $(date)"
...
trap 'echo "[llm-pom-option-loss] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
echo "[llm-pom-option-loss] Stage 2: q/k LoRA, frozen projector, loss checkpoint, POM option-ID format"
...
echo "[llm-pom-option-loss] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-pom-option-loss] done $(date)"
