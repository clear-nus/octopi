#!/bin/bash
# Queue label-order permutation augmentation after the current slotaug5 run.
# This does not regenerate QA files; augmentation is applied in the Dataset.
...
WAIT_SESSION="octopi_llm_slotaug5_gpu5"
while tmux has-session -t "$WAIT_SESSION" 2>/dev/null; do
  echo "[llm-labelorder5-3000-gpu5] waiting for tmux session $WAIT_SESSION at $(date)"
  sleep 300
done
...
RUN_CONFIG="/tmp/octopi_train_llm_labelorder5_3000_gpu5_config.yaml"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_stage1_8000_lora_qk_r128_a256_lr0.0002_freezeproj_lossval_originalpom_conclw0.25_cons0.02_labelorder5_3000_gpu5"

echo "[llm-labelorder5-3000-gpu5] start $(date)"

cp "$CONFIG_FILE" "$RUN_CONFIG"
trap 'echo "[llm-labelorder5-3000-gpu5] removing private config"; rm -f "$RUN_CONFIG"' EXIT
...
echo "[llm-labelorder5-3000-gpu5] Stage 2: q/k LoRA, original POM, conclusion_loss_weight=0.25, OPD consistency=0.02, label-order aug=5, steps=3000"
set_cfg --key data_dir --value "$DATA_DIR" \
        ...
        --key multi_object_label_order_permutation_augmentation --value 5 \
...
echo "[llm-labelorder5-3000-gpu5] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-labelorder5-3000-gpu5] done $(date)"
