#!/bin/bash
# Queue paper-aligned Stage 2 LoRA after the current LLM job finishes.
...
CONFIG_BAK="${CONFIG_FILE}.frames8_lora_qk_bak"
...
LORA_EXP_ID="full_pipeline_${SEED}_frames8enc_fullloss_lora_qk_r128_a256_lr0.0002_proj2e-5"

echo "[llm-qk] start $(date)"
echo "[llm-qk] waiting for any active train_llm.py process to finish"
while pgrep -f "python.*src/train_llm.py" >/dev/null; do
  sleep 60
done

cp "$CONFIG_FILE" "$CONFIG_BAK"
trap 'echo "[llm-qk] restoring config"; cp "$CONFIG_BAK" "$CONFIG_FILE"; rm -f "$CONFIG_BAK"' EXIT
...
if [ ! -f "$DATA_DIR/train_qa.json" ] || [ ! -f "$DATA_DIR/val_qa.json" ] || [ ! -f "$DATA_DIR/test_qa.json" ]; then
  echo "[llm-qk] regenerating data for seed ${SEED} in ${DATA_DIR}"
  rm -rf "$DATA_DIR"
...
echo "[llm-qk] Stage 2: full answer loss, LoRA target_modules=[q_proj,k_proj], r=128 alpha=256"
...
echo "[llm-qk] completed LoRA: $(latest_exp_dir "$LORA_EXP_ID")"
echo "[llm-qk] done $(date)"
