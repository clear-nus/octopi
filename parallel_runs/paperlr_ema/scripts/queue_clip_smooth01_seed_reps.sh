#!/bin/bash
# Canonical-split seed repeat for the paper-close CLIP config with
# label_smoothing=0.1. Seed 0 is expected to have been run already as
# EXP_ID=clip_seed_0_smooth01; this script records it, then runs seeds 1-4.
...
CONFIG_BAK="${CONFIG}.smooth01_seed_reps_bak"
DATASET_PATH="dataset"
DATA_DIR="data"
SUMMARY="clip_seed_reps_smooth01.txt"
CSV="${SUMMARY}.csv"
...
record_existing() {
  ...
  echo "[seed-reps] seed=${seed} existing ${exp_id} :: val_mean=${v} test_mean=${tm} test_combined=${tc}" | tee -a "$SUMMARY"
  echo "${seed},${exp_id},${v},${tm},${tc}" >> "$CSV"
}

run_seed() {
  local seed="$1" exp_id="clip_seed_${seed}_smooth01"
...
configure_smooth01
record_existing 0 "clip_seed_0_smooth01"
